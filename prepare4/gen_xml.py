...
#   SMPL-X Y (local "up")  → Newton Z (world "up", Z-up)
#   SMPL-X X               → Newton Y
...
# SMPL-X 22-joint parent chain: parent[i] is the parent joint index of joint i
# -1 means root (no parent).  Matches kintree_table from SMPLX_NEUTRAL.npz.
# Note: joints 12 (neck), 13 (L_collar), 14 (R_collar) are ALL children of
# joint 9 (spine3/Chest).  Joint 15 (head) is child of 12 (neck).
...
# Cache directories
_DEFAULT_CACHE_DIR = os.path.join(PROJECT_ROOT, "prepare4", "xml_cache")
...
def _cache_key(betas=None, foot_geom="box", bone_source="smplx", npy_path=None):
    """SHA-256 hash for cache key, covering both bone source modes."""
    parts = [bone_source.encode(), foot_geom.encode()]
...
def get_or_create_xml(betas=None, foot_geom="box", cache_dir=None,
                      bone_source="smplx", npy_path=None):
    """Get or generate+cache a per-subject XML.

    Args:
        betas: (10,) numpy array (required if bone_source="smplx")
        foot_geom: "box" | "sphere" | "capsule"
        cache_dir: directory to cache XMLs (default depends on bone_source)
    ...
    if cache_dir is None:
        cache_dir = (_DEFAULT_NPY_CACHE_DIR if bone_source == "npy"
    ...
    xml_path = os.path.join(cache_dir, f"{prefix}_{foot_geom}_{h}.xml")
